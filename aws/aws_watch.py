    ...
    for region in regions:
        ...
        for account in accounts:
            try:
                ...
                print(f"[yellow]Grabbing Names For {region}-{account['accountId']}[/yellow]")
                ...
                def sub_thread_func(name):
                    ...
                    while not kill_lock.is_set():
                        try:
                            ...
                            query_id = start_query(thread_client, name, start_time, end_time, full_query)
                            if kill_lock.is_set():
                                return
                            while True:
                                response = query_results(thread_client, query_id)
                                if response['results']:
                                    haveResults = True
                                for result in response['results']:
                                    log_entry = ' '.join([val['value'] for val in result])
                                    log = Log(log_entry)
                                    color = 'green' if log.vpc_id == 'ACCEPT' else 'red'
